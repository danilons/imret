...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--names', action="store", default='data/models/preposition-color/labels.txt')
    parser.add_argument('-d', '--deploy_file', action="store", default='data/models/preposition-color/deploy.prototxt')
    parser.add_argument('-c', '--caffemodel', action="store", default='data/models/preposition-color/snapshot.caffemodel')
    parser.add_argument('-m', '--mean_file', action="store", default='data/models/preposition-color/mean.binaryproto')
    parser.add_argument('-a', '--annot_folder', action="store", default='data/query/test_anno')
    parser.add_argument('-o', '--output_file', action="store", default='data/preposition/index-color2.csv')
    ...
    with progressbar(length=len(dset.images), show_pos=True, show_percent=True) as bar:
        for imname in dset.images:
            ...
            df = pd.DataFrame(idx, columns=['preposition', 'object1', 'object2', 'score'])
            df['image'] = len(df) * [imname]
            df.to_csv(opts.output_file, mode='a')
            bar.update(1)
